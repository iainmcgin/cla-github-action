...
describe('persistence', () => {
  ...
  describe('getFileContent', () => {
    it('GETs the signatures file from the current repo', async () => {
      http
        ...
        .reply(
          200,
          {sha: 'deadbeef', content: Buffer.from('{}').toString('base64'), encoding: 'base64'},
  ...
  describe('createFile', () => {
    it('PUTs a new signatures file with the create-file commit message', async () => {
      const captured = captureJson(
        http.github(),
        {path: '/repos/acme/widgets/contents/signatures%2Fv1%2Fcla.json', method: 'PUT'},
      ...
      expect(captured.body.message).toBe('Creating file for storing CLA Signatures')
  ...
  describe('updateFile', () => {
    it('PUTs the updated signatures file, appending newSigned contributors', async () => {
      ...
      const decoded = JSON.parse(Buffer.from(captured.body.content, 'base64').toString())
      expect(decoded.signedContributors.map((c: any) => c.name)).toEqual(['alice', 'bob'])
    ...
    it('substitutes $contributorName/$pullRequestNo/$owner/$repo in the commit message template', async () => {
      setInput('signed-commit-message', '$contributorName signed CLA on $owner/$repo#$pullRequestNo')
      const captured = captureJson(
        http.github(),
        {path: '/repos/acme/widgets/contents/signatures%2Fv1%2Fcla.json', method: 'PUT'},
        {status: 200, body: {content: {sha: 'new'}}}
      ...
      await updateFile(
        'oldsha',
        {signedContributors: []},
        {newSigned: [], allSignedFlag: false, signed: [], notSigned: [], unknown: []} as any
      )
